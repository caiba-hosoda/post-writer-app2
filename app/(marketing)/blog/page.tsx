...
const BlogPage = () => {
  ...
  return (
    <div className='container max-w-4xl py-6 lg:py-10  w-[80%] mx-auto'>
      <div>
        <div className=' space-y-4'>
          <h1 className=' font-bold text-4xl lg:text-5xl tracking-tight'>Blog</h1>
          <p className=' text-muted-foreground text-xl'>
            contentlayerとMDXでブログを作成しています。 
          </p>
        </div>
      </div>
      <hr className='my-8'/>
      ...
      <div className=' grid gap-10 sm:grid-cols-2'>
        {posts.map((post) => (
          <article key={post._id} className=' relative flex flex-col space-y-2'>
            {post.image && (<Image src={post.image} alt={post.title} width={800} height={452} className='rounded-md border bg-muted' />)}
